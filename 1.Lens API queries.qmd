...
## Read in The Lens aggregation API key and load Lens API helper function

Please first get an API key from The Lens and save the key under the 'LENS_KEY' variable in an .env file. More information on the API can be found here: https://docs.api.lens.org/aggregations-beta.html

Then, run code below to load in custom function for the calling The Lens API. I've stored the helper function code in the ''R" folder. It accepts four arguments (query_string, aggregration type, show_query_string and API token) and returns a dataframe of results. Query string refers to the Lens query string which is built from different components and are taken from the various excel file columns. There are 5 aggregation types in the helper function that provide different counts (license, oa status or oa colour as an aggregrated count or counts per year for plotting time series):
